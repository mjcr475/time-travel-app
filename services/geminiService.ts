...
// FIX: Imported the HistoricalScene type to resolve type errors.
    ...
    try {
        ...
        if (typeof error === 'object' && error !== null && 'message' in error) {
            ...
            if (/API key|authentication|UNAUTHENTICATED/i.test(errorMessage)) {
                throw new Error("Authentication Error: Please ensure your API key is correctly configured in the environment.");
            }
        }
...
export const generateRandomScenes = async (): Promise<HistoricalScene[]> =>
    withApiErrorHandling(async () => {
        // 1. Get scene ideas from text model
        const model = 'gemini-2.5-flash';
...
export const generateTimeTravelImage = (
    imageBase64: string, 
    mimeType: string, 
    prompt: string
): Promise<string | null> => 
