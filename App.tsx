...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Check for API key when the app loads
  ...
  const currentTitle = useMemo(() => {
    if (!apiKey) return "API Key Setup";
    switch (appState) {
  ...
  const loadingMessages = [ "Warming up the time machine...", "Polishing the portal lens...", "Knitting pixels through time...", "Chatting with historical experts...", "Making sure you don't step on any butterflies...", ];
  const portraitLoadingMessages = [ "Warming up the AI easel...", "Mixing digital paints...", "Sketching your likeness...", "Adding a dash of magic...", "Voila! Your masterpiece...", ];
  const sceneLoadingMessages = [ "Dreaming up new places...", "Building worlds with pixels...", "Consulting the map of imagination...", "Generating fantastical destinations...", ];
  const batchLoadingMessages = [ "Packing your bags for a big trip...", "Opening multiple time portals at once...", "Creating a whole album of memories...", "This might take a moment, great adventures await!", ];

  const renderContent = () => {
    if (!apiKey) {
      return <ApiKeyManager onKeySave={handleApiKeySave} />;
    }
    ...
    switch (appState) {
      ...
      case 'ANALYZING':
        return ( <div className="flex flex-col items-center justify-center text-center"> {userImage && <img src={`data:${userImage.type};base64,${userImage.base64}`} alt="Uploaded portrait" className="w-48 h-48 rounded-2xl object-cover mb-4 shadow-lg" />} <Spinner /> </div> );
      ...
      case 'GENERATING':
        return ( <div className="flex flex-col items-center justify-center text-center"> <div className="flex items-center justify-center space-x-4 mb-6"> {userImage && <img src={`data:${userImage.type};base64,${userImage.base64}`} alt="User portrait" className="w-32 h-32 md:w-40 md:h-40 rounded-2xl object-cover shadow-lg" />} {selectedScene && <img src={selectedScene.imageUrl} alt={selectedScene.name} className="w-32 h-32 md:w-40 md:h-40 rounded-2xl object-cover shadow-lg" />} </div> <Spinner messages={loadingMessages} /> </div> );
      case 'SHOWING_RESULT':
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 font-sans">
      <div className="w-full max-w-5xl mx-auto">
        <header className="text-center mb-6 md:mb-8">
          <div className="flex items-center justify-center gap-3 mb-2"> <LogoIcon /> <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight text-gray-800"> Time-Travel Booth </h1> </div>
        ...
        <footer className="text-center mt-8 text-gray-500"> <p>Powered by Google Gemini. Have fun creating moments out of time!</p> </footer>
      </div>
    </div>
  );
};
